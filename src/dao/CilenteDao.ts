...
class ClienteDao {
    ...
    public static async crearCliente(email: string, telefono: string, direccion: string, password: string,
        nombre: string,
        apellidos: string,
    ): Promise<any> {
        try {
            ...
            const nuevoCliente = new ClienteEntidad(email, telefono, direccion, contra, nombre, apellidos, nombreRolPorDefecto);
